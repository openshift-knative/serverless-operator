...
func (e *extension) cleanupDomainMapping(ctx context.Context, ks *operatorv1beta1.KnativeServing) error {
	...
	if err := client.CoreV1().Services(ks.GetNamespace()).Delete(ctx, "domainmapping-webhook", metav1.DeleteOptions{}); err != nil && !apierrors.IsNotFound(err) {
		return fmt.Errorf("failed to delete service domainmapping-webhook: %w", err)
	}
