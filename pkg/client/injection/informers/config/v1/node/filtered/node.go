...
func Get(ctx context.Context, selector string) v1.NodeInformer {
	...
	if untyped == nil {
		logging.FromContext(ctx).Panicf(
			"Unable to fetch github.com/openshift-knative/serverless-operator/pkg/client/informers/externalversions/config/v1.NodeInformer with selector %s from context.", selector)
	}
...
var _ configv1.NodeLister = (*wrapper)(nil)

func (w *wrapper) Informer() cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(nil, &apiconfigv1.Node{}, 0, nil)
}

func (w *wrapper) Lister() configv1.NodeLister {
	return w
}

func (w *wrapper) List(selector labels.Selector) (ret []*apiconfigv1.Node, err error) {
	reqs, err := labels.ParseToRequirements(w.selector)
	if err != nil {
		return nil, err
	}
	selector = selector.Add(reqs...)
	lo, err := w.client.ConfigV1().Nodes().List(context.TODO(), metav1.ListOptions{
...
func (w *wrapper) Get(name string) (*apiconfigv1.Node, error) {
	// TODO(mattmoor): Check that the fetched object matches the selector.
	return w.client.ConfigV1().Nodes().Get(context.TODO(), name, metav1.GetOptions{
		// TODO(mattmoor): Incorporate resourceVersion bounds based on staleness criteria.
	})
}
