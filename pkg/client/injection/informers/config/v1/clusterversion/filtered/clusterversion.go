...
func Get(ctx context.Context, selector string) v1.ClusterVersionInformer {
	untyped := ctx.Value(Key{Selector: selector})
	if untyped == nil {
		logging.FromContext(ctx).Panicf(
			"Unable to fetch github.com/openshift-knative/serverless-operator/pkg/client/informers/externalversions/config/v1.ClusterVersionInformer with selector %s from context.", selector)
	}
