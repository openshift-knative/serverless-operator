...
func TestKnativeServing(t *testing.T) {
	...
	t.Run("verify correct deployment shape", func(t *testing.T) {
		...
		for _, deployment := range []string{"activator", "controller", "autoscaler-hpa", "domain-mapping"} {
		...
		for _, deployment := range []string{"activator", "autoscaler", "autoscaler-hpa", "controller", "domain-mapping", "webhook"} {
