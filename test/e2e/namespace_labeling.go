...
func VerifyNamespaceMetadata(t *testing.T, caCtx *test.Context, namespace string) {
	...
	t.Run("Namespace "+namespace+" additional labels and annotations", func(t *testing.T) {
		t.Run("Add labels", func(t *testing.T) {
			if err := ensureMetadataOnNamespace(caCtx, ctx, namespace, labels, labelKVMetadata); err != nil {
				t.Fatal(err)
			}
			if err := verifyMetadataOnNamespace(caCtx, ctx, namespace, labels, labelKVMetadata); err != nil {
				t.Fatal(err)
			}
		})

		t.Run("Add annotations", func(t *testing.T) {
			if err := ensureMetadataOnNamespace(caCtx, ctx, namespace, annotations, annotationsKVMetadata); err != nil {
				t.Fatal(err)
			}
			if err := verifyMetadataOnNamespace(caCtx, ctx, namespace, annotations, annotationsKVMetadata); err != nil {
				t.Fatal(err)
			}
		})
	})
...
func ensureMetadataOnNamespace(caCtx *test.Context, ctx context.Context, namespace string, additional map[string]string, getKVMetadata getKVMetadata) error {
