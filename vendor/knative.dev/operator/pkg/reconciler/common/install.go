...
func Install(ctx context.Context, manifest *mf.Manifest, instance base.KComponent) error {
	...
	if err := manifest.Filter(mf.Any(mf.ByKind("ConfigMap"))).Apply(); err != nil {
		status.MarkInstallFailed(err.Error())
		return fmt.Errorf("failed to apply configmaps: %w", err)
	}
	if err := manifest.Filter(mf.Any(mf.ByKind("Deployment"))).Apply(); err != nil {
		status.MarkInstallFailed(err.Error())
		return fmt.Errorf("failed to apply deployments: %w", err)
	}
