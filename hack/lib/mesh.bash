...
function deploy_servicemeshcontrolplane {
  ...
  timeout 120 "[[ \$(oc apply -f ${resources_dir}/istio.yaml | oc get istios -n istio-system knative-istio --no-headers | wc -l) != 1 ]]" || return 1
...
#  fi
  ...
  oc wait deployment --all --timeout=600s --for=condition=Available -n istio-cni || oc get deployment  --all -n istio-cni -o yaml
}

function undeploy_servicemeshcontrolplane {
  logger.info "Deleting istiod"
  oc delete istios knative-istio -n istio-system --ignore-not-found || return $?
