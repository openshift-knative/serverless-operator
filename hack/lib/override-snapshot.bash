...
readonly component_override_snapshot_filename="override-snapshot.yaml"

function get_fbc_snapshot_filename {
  local ocp_version
  ocp_version="${1}"

  echo "override-snapshot-fbc-${ocp_version}.yaml"
}
...
function create_component_snapshot {
  ...
  snapshot_file="${1}/${component_override_snapshot_filename}"
...
function create_fbc_snapshots {
  ...
  while IFS= read -r ocp_version; do
    ...
    snapshot_file="${snapshot_dir}/$(get_fbc_snapshot_filename "${ocp_version}")"
...
function verify_component_snapshot {
  local snapshot_file repo revision component repo_revision rc
  snapshot_file="${1}/${component_override_snapshot_filename}"
...
function verify_fbc_snapshots {
    local rootdir snapshot_dir fbc_revision rc
    rootdir="$(dirname "$(dirname "$(dirname "$(realpath "${BASH_SOURCE[0]}")")")")"
    snapshot_dir="${1}"
    rc=0
    fbc_revision=""
    ...
    while IFS= read -r ocp_version; do
      ...
      current_fbc_revision="$(yq read "${rootdir}/${snapshot_file}" "spec.components[0].source.git.revision")" # FBC snapshot have only 1 component, so taking index 0 is fine here
      if [[ -z "$fbc_revision" ]]; then
        # no revision set so far
        fbc_revision="$current_fbc_revision"
        continue
      else
        if [[ "$fbc_revision" != "$current_fbc_revision" ]]; then
          # revision mismatch
          echo "Revision of FBC for OCP ${ocp_version} didn't match. Expected revision ${fbc_revision}, but got ${current_fbc_revision}"
          rc=1
        fi
      fi
