...
function add_upstream_operator_deployment_image {
  yq e --inplace "(.spec.install.spec.deployments[] | select(.name == \"knative-operator\").spec.template.spec.containers[] | select(.name == \"knative-operator\").env) += {\"name\": \"${2}\", \"value\": \"${3}\"}" "$1"
}
