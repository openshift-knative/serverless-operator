...
test-upstream-e2e-mesh:
	...
	TRACING_BACKEND=zipkin TRACING_NAMESPACE=knative-eventing ./hack/tracing.sh
	UNINSTALL_STRIMZI="false" ./hack/strimzi.sh
	FULL_MESH=true SCALE_UP=5 INSTALL_SERVING=true INSTALL_EVENTING=false INSTALL_KAFKA=false TRACING_BACKEND=zipkin TRACING_NAMESPACE=knative-eventing ENABLE_TRACING=true ./hack/install.sh
	FULL_MESH=true ./test/e2e-tests.sh
	FULL_MESH=true TEST_KNATIVE_KAFKA=false TEST_KNATIVE_SERVING=true TEST_KNATIVE_EVENTING=false TEST_KNATIVE_KAFKA_BROKER=false TEST_KNATIVE_UPGRADE=false ./test/upstream-e2e-tests.sh
