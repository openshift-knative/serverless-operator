FROM registry.ci.openshift.org/origin/scos-__OCP_BUILD_VERSION__:cli-artifacts AS oc-image

FROM src

ARG TARGETARCH

COPY --from=oc-image /usr/share/openshift/linux_$TARGETARCH/oc.rhel9 /usr/bin/oc
